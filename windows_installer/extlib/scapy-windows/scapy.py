...
DEFAULT_CONFIG_FILE = os.path.join(os.path.expanduser('~'), ".scapy_startup.py")
...
if not PCAP:
    # Note: Testing still. On Windows pcap is mandatory at the moment so PCAP 
    # is always True and this code will not be executed.
    if WINDOWS:
        f = os.popen("WinDump -V 2> NUL:")
    f = os.popen("tcpdump -V 2> /dev/null")
    if f.close() >> 8 == 0x7f:
        if WINDOWS:
            #Better Error Info for Windows maybe
            log_loading.warning("Failed to execute WinDump. Check it is installed and in the PATH")
        else:
...
try:
    ...
    def inet_aton(x):
        if WINDOWS:
            if "/" in str(x):
                x=str(x).split("/")[0]
            if str(x) == "255.255.255.255":
                return "\xff"*4
            else:
                return socket.inet_aton(str(x))
        else:
        ...
        if af == socket.AF_INET:
            ...
            for part in parts:
                if part == JOKER:
                    ...
                    try:
                        result += binascii.unhexlify(part.rjust(4, "0"))
                    except TypeError:
                        raise Exception("Illegal syntax for IP address")
...
class Console(object):
    """Console with ANSI color support on Windows
    
    Printing to an instance of this class (instead of sys.stdout) gives color
    support on the Windows console."""
    ...
    def write(self, text):
        if WINDOWS and self._colored:
            # XXX: Windows own console output handling has problems with strings 
            # longer than 64k. So if our output text is really long, we have to 
            # split it into several parts and print each one separately.
            # (Eventually this should be handled in pyreadline though.)
            maxlen = 10000    # conservative guess to make room for Unicode conversion
            # FIXME: In corner cases this could erroneously split ANSI-sequences
            parts = [text[start:start+maxlen] for start in range(0, len(text), maxlen)]
            for text in parts:
...
# XXX: Redirecting stdout ('sys.stdout = console') would break readline support
# (e.g. tab completion) so we have to replace every "print" with 
# "print >> console" explicitly. Don't mix these calls (at least per function)
# or subtle problems with spacing and line handling will appear.
...
def hexdump(x):
    ...
    while i < l:
        ...
        for j in range(16):
            if i+j < l:
                print >> console, "%02X" % ord(x[i+j]),
            else:
                print >> console, "  ",
            if j%16 == 7:
                print >> console, "",
        print >> console, " ",
        print >> console, sane_color(x[i:i+16])
        i += 16
...
def linehexdump(x, onlyasc=0, onlyhex=0):
    ...
    if not onlyasc:
        for i in range(l):
            print >> console, "%02X" % ord(x[i]),
        print >> console, "",
    if not onlyhex:
        print >> console, sane_color(x)

def chexdump(x):
    x=str(x)
    print >> console, ", ".join(map(lambda x: "%#04x"%ord(x), x))
    ...
    while i < l:
        ...
        if dox:
            ...
            while not linex[j]:
                j += 1
                xd -= 1
            print >> console, colorize[doy-dox]("%04x" % xd),
            x += xx
            line=linex
        else:
            print >> console, "    ",
        if doy:
            ...
            while not liney[j]:
                j += 1
                yd -= 1
            print >> console, colorize[doy-dox]("%04x" % yd),
            y += yy
            line=liney
        else:
            print >> console, "    ",
        ...
        for j in range(16):
            if i+j < l:
                if line[j]:
                    ...
                    print >> console, col("%02X" % ord(line[j])),
                    if linex[j]==liney[j]:
...
def warning(x):
    # XXX: The warning message might contain ANSI color codes, so on Windows
    # we print to our special ANSI console object. This bypasses the logging
    # system and breaks conf.warning_threshold filter. Better solution wanted.
    if WINDOWS:
        print >> console, "WARNING: %s" % x
    else:
...
def do_graph(graph,prog=None,format=None,target=None,type=None,string=None,options=None):
    ...
    target: filename or redirect. Default: show with associated viewer
...
if 0 and DNET: ## XXX Can't use this because it does not resolve IPs not in cache
    ...
    def getmacbyip(ip, chainCC=0):
        ...
        ifip = str(dnet.intf().get(iff)['addr'])
        if gw != ifip.split('/')[0]:
            ip = gw
...
class ASN1_Object:
    ...
    def show(self, lvl=0):
        print >> console, self.strshow(lvl)
    def __eq__(self, other):
...
lfilter: truth function to apply to each packet to decide whether it will be displayed"""
        for i in range(len(self.res)):
            if lfilter is not None:
                if not lfilter(self.res[i]):
                    continue
            print >> console, conf.color_theme.id(i,"%04i"),
            if prn is None:
                print >> console, self._elt2sum(self.res[i])
            else:
                print >> console, prn(self.res[i])
    ...
    def hexraw(self, lfilter=None):
        ...
        for i in range(len(self.res)):
            ...
            if lfilter is not None and not lfilter(p):
                continue
            print >> console, "%s %s %s" % (conf.color_theme.id(i,"%04i"),
                                p.sprintf("%.time%"),
    ...
    def psdump(self, filename = None, **kargs):
        ...
        if filename is None:
            if WINDOWS:
                filename = os.tempnam("", "scapy.psd.")
            else:
                filename = "/tmp/scapy.psd.%i" % os.getpid()
            d.writePSfile(filename)
            if WINDOWS:
                if conf.prog.psreader == conf.prog._default:
                    os.startfile(filename+".ps")
                else:
    ...
    def pdfdump(self, filename = None, **kargs):
        ...
        if filename is None:
            ...
            if WINDOWS:
                if conf.prog.pdfreader == conf.prog._default:
    ...
    def trace3D(self):
        ...
        while 1:
            if visual.scene.kb.keys:
                ...
                if k == "esc" or (WINDOWS and k == "q"):  # esc does not work on Windows
                    break
...
Creates an EPS file describing a packet. If filename is not provided a temporary file is created and gs is called."""
        ...
        if filename is None:
            if WINDOWS:
                fname = os.tempnam("", "scapy.psd.")
            else:
                fname = "/tmp/scapy.%i"%os.getpid()
            canvas.writeEPSfile(fname)
            if WINDOWS:
                if conf.prog.pdfreader == conf.prog._default:
                    os.startfile(fname+".eps")
                else:
...
#        backcolor=makecol(0.376, 0.729, 0.525, 1.0)
    ...
    def show(self, indent=3, lvl="", label_lvl=""):
        ...
        ct = conf.color_theme
        print >> console, "%s%s %s %s" % (label_lvl,
                              ct.punct("###["),
        ...
        for f in self.fields_desc:
            ...
            if isinstance(fvalue, Packet) or (f.islist and f.holds_packets and type(fvalue) is list):
                print >> console, "%s  \\%-10s\\" % (label_lvl+lvl, ncol(f.name))
                fvalue_gen = SetGen(fvalue,_iterpacket=0)
                for fvalue in fvalue_gen:
                    fvalue.show(indent=indent, label_lvl=label_lvl+lvl+"   |")
            else:
                print >> console, "%s  %-10s%s %s" % (label_lvl+lvl,
                                                      ncol(f.name),
                                                      ct.punct("="),
...
class L3dnetSocket(SuperSocket):
    def __init__(self, type = ETH_P_ALL, filter=None, promisc=None, iface=None, nofilter=0):
        if iface is None:
            iface = conf.iface  # must be set before the pcap_name(iface) call below
        self.iface = iface
        self.iflist = {}
        if WINDOWS:
            self.ins = pcap.pcap(name=pcap_name(iface), snaplen=1600, promisc=0, timeout_ms=500)
        else:
    ...
    def send(self, x):
        ...
        if ifs is None:
            try:
                ...
                logmsg = "Could not set interface. This packet will NOT be sent (don't trust the packet count)."
                if WINDOWS:
                    logmsg += " Sending packets to localhost is not supported on Windows."
                warning(logmsg)
                return
        sx = str(Ether()/x)
        x.sent_time = time.time()        
    ...
    def recv(self,x=MTU):
        ...
        if pkt is not None:
            if WINDOWS:
                ts, pkt = pkt  # pypcap returns timestamp first
            else:
                l,pkt,ts = pkt
        if pkt is None:
            return
...
class L2pcapListenSocket(SuperSocket):
    def __init__(self, iface = None, type = ETH_P_ALL, promisc=None, filter=None):
        ...
        self.promisc = promisc
        if WINDOWS:
            self.ins = pcap.pcap(name=pcap_name(iface), snaplen=1600, promisc=self.promisc, timeout_ms=500)
        else:
            self.ins = pcap.pcapObject()
...
def sendpfast(x, pps=None, mbps=None, realtime=None, loop=0, iface=None):
    ...
    try:
        try:
            if WINDOWS:
                subprocess.Popen([conf.prog.tcpreplay] + options)
            else:
    ...
    try:
        while 1:
            ...
            if count is not None:
                ...
                count -= 1
            start = time.time()
            print >> console, "\rsend...\r",
            res = srfunc(pkts, timeout=timeout, verbose=0, chainCC=1, *args, **kargs)
            n += len(res[0])+len(res[1])
            r += len(res[0])
            if verbose > 1 and prn and len(res[0]) > 0:
                msg = "RECV %i:" % len(res[0])
                print >> console, "\r"+ct.success(msg),
                for p in res[0]:
                    print >> console, col(prn(p))
                    print >> console, " "*len(msg),
            if verbose > 1 and prnfail and len(res[1]) > 0:
                msg = "fail %i:" % len(res[1])
                print >> console, "\r"+ct.fail(msg),
                for p in res[1]:
                    print >> console, col(prnfail(p))
                    print >> console, " "*len(msg),
            if verbose > 1 and not (prn or prnfail):
                print >> console, "recv:%i  fail:%i" % tuple(map(len, res[:2]))
            if store:
                ans += res[0]
                unans += res[1]
            end=time.time()
...
def sndrcvflood(pks, pkt, prn=lambda (s,r):r.summary(), chainCC=0, store=1, unique=0):
    ...
    try:
        while 1:
            ...
            if WINDOWS or ssock in readys:
                pks.send(packets_to_send.next())
                
            if WINDOWS or rsock in readyr:
                p = pks.recv(MTU)
                if p is None:
                    continue
                h = p.hashret()
...
cPickle.dump(db, gzip.open("GeoIPCountry4Scapy.gz","w"))
"""
    def lazy_init(self):
...
def snmpwalk(dst, oid="1", community="public"):
    try:
        while 1:
            ...
            if ICMP in r:
                print >> console, repr(r)
                break
            if r is None:
                print >> console, "No answers"
                break
            print >> console, "%-40s: %r" % (r[SNMPvarbind].oid.val,r[SNMPvarbind].value)
...
user_commands = [ sr, sr1, srp, srp1, srloop, srploop, sniff, p0f, arpcachepoison, send, sendp, traceroute, arping, ls, lsc, queso, nmap_fp, report_ports, dyndns_add, dyndns_del, is_promisc, promiscping, show_interfaces ]
...
"""
    ...
    if WINDOWS:
        ...
        nmap_base = os.path.join(os.environ["ProgramFiles"], "nmap",
...
#    __builtin__.__dict__.update(scapy.__dict__)
    ...
    sys.ps1 = ColorPrompt()
    if WINDOWS:
        # We colorize expressions which are evaluated in interactive sessions (PEP 217)
        import __builtin__
        def echo_to_console(obj):
            if obj is None:
                return
            __builtin__._ = None
            r = repr(obj)
            print >> console, r
            __builtin__._ = obj
        sys.displayhook = echo_to_console
